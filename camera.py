...
screen.fill(WHITE)
pygame.display.set_caption("kimbab")
### window 사이즈
...
def push_rst():
    global order, order_list, price_num
    order_list = []
    order = []
    price_num = 1000
...
def push_bab():
    global flag_v
    global order, price_num, now_price_num
...
def start():
    ...
    while True:
        ...
        fav_img = pygame.image.load("fav.png")
        ...
        #chu = game_font.render(chuMenu, True, (255, 255, 255))
        #screen.blit(chu, (10,1400))
        pygame.display.update()
        for event in pygame.event.get():
