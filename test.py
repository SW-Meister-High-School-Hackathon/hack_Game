...
# 다시 start페이지로 가는거임
def push_back():
    global flag_back, order, order_list, price_num, max_stuff, cam_stuff, che_stuff, dan_stuff, dang_stuff, don_stuff, oi_stuff, ung_stuff, egg_stuff, ham_stuff, mat_stuff, flag_next
    flag_back = 1
    flag_next = 0
...
# 랭킹 쪽 부분
def rank():
    global url_ranking, flag_next, flag_rank, flag_start, flag_back, oi_stuff, dan_stuff, ham_stuff, don_stuff, ung_stuff, che_stuff, cam_stuff,egg_stuff, mat_stuff, dang_stuff
    global order, order_list, price_num, now_price_num, flag_v, max_stuff
    order = []
    order_list = []
    price_num = 1000
    now_price_num = 0
...
def start():
    ...
    while True:
        ...
        if flag_start == 0 and flag_back == 0:
            break
